...
export default function HistoryPage() {
  ...
  useEffect(() => {
    usersApi.history().then((r) => setProperties(r.data)).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        <div>
          <h1 style={S.title}>История просмотров</h1>
          <p style={S.sub}>{properties.length > 0 ? `${properties.length} просмотренных объектов` : "Недавно просмотренные объекты"}</p>
        </div>
      </div>
      ...
      {!loading && properties.length === 0 && (
        <div style={S.empty}>
          ...
          <h3 style={S.emptyTitle}>История пуста</h3>
          <p style={S.emptySub}>Здесь появятся объекты, которые вы просматривали</p>
        </div>
      )}
    </div>
  );
...
const S = {
  ...
  icon: {
    width: 56, height: 56, background: "#F0F9FF", borderRadius: 16,
